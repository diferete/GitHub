...
class ViewSTEEL_PCP_GerenFat extends View {
    ...
    public function criaTela() {
        ...
        $oGridFatDiario = new Campo('Fat. Diário ' . date('d/m/Y'), 'fatDir', Campo::TIPO_GRIDVIEW, 6, 6, 12, 12);
        $oGridFatDiario->setSTituloGridPainel('Faturamento diário ' . date('d/m/Y'));
        ...
        $oGridFatPeso->setSTituloGridPainel('Faturamento mensal de ' . $oDataIni->getSValor() . ' até ' . $oDataFim->getSValor());
        ...
        $oGridFatPesoAcabado->setSTituloGridPainel('Produtos acabados mensal de ' . $oDataIni->getSValor() . ' até ' . $oDataFim->getSValor());
        ...
        $oGridFatPesoFio->setSTituloGridPainel('Fio máquina mensal de ' . $oDataIni->getSValor() . ' até ' . $oDataFim->getSValor());
        ...
        $this->getTela()->setSAcaoShow('requestAjax("' . $this->getTela()->getId() . '-form","STEEL_PCP_GerenFat",'
                . '"carregaDadosGerenFat","' . $oGridFatDiario->getId() . ',' . $oGridFatPeso->getId() . ',' . $oGridFatPesoFio->getId() . ',' . $oGridFatPesoAcabado->getId() . '");');

        //adiciona evento no botao
        $sEventoOp = ' var dataini = $("#' . $oDataIni->getId() . '").val(); var datafim = $("#' . $oDataFim->getId() . '").val();'
                . ' $("#' . $oGridFatPeso->getId() . '-spanvalor").text("Faturamento mensal de "+dataini+" até "+datafim); '
                . ' $("#' . $oGridFatPesoAcabado->getId() . '-spanvalor").text("Produtos acabados mensal de "+dataini+" até "+datafim); '
                . ' $("#' . $oGridFatPesoFio->getId() . '-spanvalor").text("Fio máquina mensal de "+dataini+" até "+datafim); '
                . 'requestAjax("' . $this->getTela()->getId() . '-form","STEEL_PCP_GerenFat",'
        ...
        $this->addCampos($oField2, $oLinha, array($oGridFatDiario, $oGridFatPeso), array($oGridFatPesoFio, $oGridFatPesoAcabado));
    }
