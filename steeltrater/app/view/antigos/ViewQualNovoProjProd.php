...
class ViewQualNovoProjProd extends View{
    ...
    private $resp_venda_nome;
        ...
        $oFSitProj = new Filtro($oSitGeral, Filtro::CAMPO_SELECT,2,2,2,2);
        ...
        $aInicial[0]='sitgeralproj,Lib.Cadastro';
    ...
    public function criaTelaProd() {
        ...
        $oFieldDimen->addCampos(array($oChaveMin,$oChaveMax,$oAltMin,$oAltMax),array($oDiamFmin,$oDiamFmax,$oCompMin,$oCompMax),
                array($oDiamPriMin,$oDiamPriMax,$oDiamExtMin,$oDiamExtMax),array($oCompRMin,$oCompRMax,$oCompHasteMin,$oCompHasteMax),
                array($oDiamHasteMin,$oDiamHasteMax,$oAngHelice,$oAcab,$oMaterial,$oClasse));
    ...
    private $normapropmec;*/
       ...
       $oReqCli = new Campo('Requisito adicional solicitado pelo cliente','reqcli', Campo::TIPO_TEXTAREA,6,6,12,12);
        ...
        $oFieldAnaliseCri->addCampos(array($oLabel1,$oLabel3),array($oDadosEnt,$oDadosEnt_obs),
                array($oReqLegal,$oReqLegal_obs),array($oReqadicional,$oReqadicional_obs),
                array($oReqadverif,$oReqadverif_obs),array($oReqadval,$oReqadval_obs),
                array($oReqproblem,$oReqproblem_obs),$oComen);
       
        $this->addCampos(array($oFilcgc,$oNr,$oDtimp,$oResp_proj_nome,$oResp_venda_nome,$oSitgeralproj),
                $oObsCli,array($oProcod,$oDesc_novo_prod),array($oProcodSimilar,$oProdsimilar),$oFieldDimen,
                array($oTipRosca,$oNormaDimem),array($oNormaRosca,$oNormaMec),array($oPpap,$oVolVenda),$oReqCli,
               array($oRespProj,$oRespProjNome,$oDataProd),$oFieldAnaliseCri);
