...
class ViewMET_QUAL_AqPlanApont extends View {
    ...
    public function criaTela() {
        ...
        $oGridAq->getOGrid()->setSEventoClick('var chave=""; $("#' . $oGridAq->getId() . ' tbody .selected").each(function(){chave = $(this).find(".chave").html();}); '
                . 'requestAjax("","MET_QUAL_AqPlanApont","sendaDadosCampos","' . $oGridAq->getId() . '"+","+chave+","+"' . $oSeqEnv->getId() . '"+","+"' . $oPlanEnv->getId() . '"+","+"' . $oDataFim->getId() . '"+","+"' . $oObsFim->getId() . '"); ');

        $sAcaoBusca = 'requestAjax("' . $this->getTela()->getId() . '-form","MET_QUAL_AqPlanApont","getDadosGrid","' . $oGridAq->getId() . '","criaConsutaApont"); ';
