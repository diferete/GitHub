...
class ViewMET_QUAL_Ata extends View {
    ...
    public function criaTela() {
        ...
        $sAcaoBusca = 'requestAjax("' . $this->getTela()->getId() . '-form","MET_QUAL_Ata","getDadosGrid","' . $oGridAta->getId() . '","consultaAta");';
        ...
        $sAcao = 'var chave=""; $("#' . $oGridAta->getId() . ' tbody .selected").each(function(){chave = $(this).find(".chave").html();}); '
                . 'requestAjax("' . $this->getTela()->getId() . '-form","MET_QUAL_Ata","excluirAta","' . $this->getTela()->getId() . '-form,' . $oGridAta->getId() . '"+","+chave+""); '; // excluirEf
