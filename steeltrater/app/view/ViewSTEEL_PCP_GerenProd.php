...
class ViewSTEEL_PCP_GerenProd extends View {
    ...
    public function criaTela() {
        ...
        $this->getTela()->setSAcaoShow('requestAjax("' . $this->getTela()->getId() . '-form","STEEL_PCP_GerenProd",'
                . '"carregaDadosGerenProd","' . $oGridProdDiario->getId() . ',' . $oGridProdMensal->getId() . ',' . $oGridProdFornoDiario->getId() . ',' . $oGridProdFornoMensal->getId() . '");');

        //adiciona evento no botao
        $sEventoOp = ' var dataini = $("#' . $oDataIni->getId() . '").val(); var datafim = $("#' . $oDataFim->getId() . '").val();'
                . ' $("#' . $oGridProdMensal->getId() . '-spanvalor").text("Produção de "+dataini+" até "+datafim); '
        ...
        $this->addCampos($oField2, $oLinha, array($oGridProdDiario, $oGridProdMensal), array($oGridProdFornoDiario, $oGridProdFornoMensal));
    }
