...
class ControllerMET_QUAL_Contencao extends Controller {

    public function __construct() {
        $this->carregaClassesMvc('MET_QUAL_Contencao');
        $this->setControllerDetalhe('MET_QUAL_Correcao');
        $this->setSMetodoDetalhe('criaPainelCorrecao');
    }
     ...
     * @param type $sCampos
     */
    public function criaPainelContencao($sDados, $sCampos) {
    ...
    public function afterDelete() {
        ...
        $this->Persistencia->deletaContencao($this->Model->getFilcgc(), $this->Model->getNr(), $this->Model->getSeq());
        $aRetorno[0] = true;
    ...
    public function criaTelaModalApontaContencao($sDados) {
    ...
    public function apontaContencao($sDados) {
        ...
        if ($aCampos['dtaponta'] != null && $aCampos['apontamento'] != null) {
            $aRet = $this->Persistencia->apontaContencao();
            if ($aRet[0]) {
    ...
    public function apontaRetAberto($sDados) {
        ...
        if ($aCampos['seq'] == '') {
            ...
            $aRet = $this->Persistencia->retContencao();
            if ($aRet[0]) {
