...
/*
 * Implementa a classe controler MET_SOL_AprovacoesItens
 * @author Alexandre de Souza
 * @since 18/08/2021
 */

class ControllerMET_SOL_AprovacoesItens extends Controller {

    public function __construct() {
        $this->carregaClassesMvc('MET_SOL_AprovacoesItens');
    }
    ...
    public function TelaVisualizaItens($sDados) {
        ...
        $oMET_SOL_Aprovacoes = Fabrica::FabricarController('MET_SOL_Aprovacoes');
        $oMET_SOL_Aprovacoes->Persistencia->adicionaFiltro('filcgc', $aCamposChave['filcgc']);
        $oMET_SOL_Aprovacoes->Persistencia->adicionaFiltro('solcod', $aCamposChave['solcod']);
        $oDados = $oMET_SOL_Aprovacoes->Persistencia->consultarWhere();
    ...
    public function gravaQnt($sDados) {
        ...
        $aRetorno = $this->Persistencia->alteraQt($aDados[2], $this->Model->getFilcgc(), $this->Model->getSolcod(), $this->Model->getSup_solicitacaoitemseq());
