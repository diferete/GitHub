...
class ControllerSTEEL_PCP_GerenProd extends Controller {
    ...
    public function carregaDadosGerenProd($sDados) {
        ...
        $aDadosParam['dataini'] = Util::getDataAtual();
        $aDadosParam['datafin'] = Util::getDataAtual();
        $aDadosParam['tipoOp'] = "'P','TZ'"; //SOMENTE ENTRA OP FINALIZADA
        ...
        $sHtmlDiario = '             <tr><td>Produção total</td><td> ' . number_format($aDadosPesoGeral['pesoTotal'], 2, ',', '.') . ' Kg</td></tr> '
                ...
                // .'             <tr><td>Zincagem</td><td>'.number_format($aDadosPesoZincagem['pesoTotal'], 2, ',', '.').' Kg</td></tr> '
        ...
        $sHtmlDiario = '             <tr><td>Produção total</td><td> ' . number_format($aDadosPesoGeralMensal['pesoTotal'], 2, ',', '.') . ' Kg</td></tr> '
                ...
                // .'             <tr><td>Zincagem</td><td>'.number_format($aDadosPesoZincMes['pesoTotal'], 2, ',', '.').' Kg</td></tr> '
                . '<tr><td colspan="2" align="center" style="color:red;font-size:14px;background:#f3f7f9">Produção por etapas</td></tr>';
        foreach ($aDadosEtapa as $keyEtapa => $ValueEtapa) {
            $sHtmlDiario .= '<tr><td>' . $keyEtapa . '</td><td> ' . number_format($ValueEtapa, 2, ',', '.') . ' Kg</td></tr>';
        }
        ...
        $aDadosParam['datafin'] = Util::getDataAtual();
        $aDadosParam['tipoOp'] = "'P','TZ'";
        $aDadosProdFornoDiario = $this->Persistencia->geraGerenProdEtapaForno($aDadosParam);
        //$aDadosProdFornoDiario = $this->Persistencia->geraGerenProd($aDadosParam); //MÉTODO ANTERIOR 16.07
        $sHtmlDiario = '';
        foreach ($aDadosProdFornoDiario as $key => $value) {
            $sHtmlDiario .= '             <tr><td>' . $key . ' </td><td> ' . number_format($value, 2, ',', '.') . ' Kg Etapa !Atenção</td></tr> ';
        }
        ...
        $aDadosProdFornoMensal = $this->Persistencia->geraGerenProdEtapaForno($aDadosParam);
        //$this->Persistencia->geraGerenProd($aDadosParam); 
    ...
    public function getProdAppSteel($sDataIni, $sDataFim) {
        ...
        $aDados['ProdMensal']['DadosEtapaZinc'] = $aDadosEtapaZincMensal['LINHA DE ZINCAGEM 1'] + $aDadosEtapaZincMensal['LINHA DE ZINCAGEM 2'] . '.0000';
