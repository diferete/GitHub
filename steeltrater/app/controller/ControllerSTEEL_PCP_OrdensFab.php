...
class ControllerSTEEL_PCP_OrdensFab extends Controller {
    ...
    public function mostraTelaRelOpEmitida($renderTo, $sMetodo = '') {
        $parame = $this->Persistencia->buscaTratamento();
    ...
    public function verificaCampos($sAcao) {
        ...
        $iContProdFin = $oCodProdFin->Persistencia->getCount();
        if ($iContProdFin > 0) {
            $oProdResultFin = $oCodProdFin->Persistencia->consultarWhere();
            $this->Model->setProdesFinal($oProdResultFin->getPro_descricao());
        } else {
            $oModal = new Modal('Atenção', 'Esse produto final não está no seu cadastro de produtos!', Modal::TIPO_ERRO);
            echo $oModal->getRender();
            exit();
        }
        ...
        if ($this->Model->getEmp_codigo() == '16746136000185' || $this->Model->getEmp_codigo() == '84229624000175' || $this->Model->getEmp_codigo() == '84229624000418') {
            ...
            if ($this->Model->getXPed() == null || $this->Model->getNItemPed() == null || $this->Model->getNItemPedServico() == null || $this->Model->getNItemPedInsumo() == null || $this->Model->getNItemPedEnergia() == null) {
                $oMensagem = new Mensagem('Atenção, esse cliente exige XPED e NITEMPED', 'Atenção fique atento........', Mensagem::TIPO_ERROR, 10000);
                echo $oMensagem->getRender();
                exit();
            }
        }
    ...
    /**
     * Modal apontamentos de peso
     */
    public function criaTelaModalApontaPeso($sDados) {
        $this->View->setSRotina(View::ACAO_ALTERAR);
        $aDados = explode(',', $sDados);
        $aChave = explode('&', $aDados[2]);
        $aOp = explode('=', $aChave[0]);

        $this->View->criaModalPeso($aOp);
        //busca lista pela op
    ...
    public function atualizaPesoBal($sDados) {
        $sTela = $sDados;
        $aDados = $this->getArrayCampostela();

        //ValorSql($valor)

        $aDados['peso'] = $this->ValorSql($aDados['peso']);
        $aDados['pesobal'] = $this->ValorSql($aDados['pesobal']);
        $aDados['pesocaixa'] = $this->ValorSql($aDados['pesocaixa']);
        $aDados['pesodif'] = $this->ValorSql($aDados['pesodif']);

        $aReturn = $this->Persistencia->pesoBalancaoOp($aDados);
        if ($aReturn[0] == true) {
            $oMenSuccess = new Mensagem("Sucesso", "Seu peso foi inserido com sucesso!", Mensagem::TIPO_SUCESSO);
            echo $oMenSuccess->getRender();
        } else {
            $oMenSuccess = new Mensagem("Atenção", "Verifique seus dados!", Mensagem::TIPO_WARNING);
            echo $oMenSuccess->getRender();
        }
    }
    ...
    public function relatorioExcelProducao() {
        ...
        $sSistema = "app/relatorio";
        $sRelatorio = 'RelProducaoExcel.php?';
