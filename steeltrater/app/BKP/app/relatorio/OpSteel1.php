...
foreach ($aOps as $key => $aOp) {
   ...
   //busca itens do tratamento
    ...
    $sSqlMaterial = "select seqmat,STEEL_PCP_PRODMATRECEITA.matcod,matdes,durezaNucMin,durezaNucMax,
                    durezaSuperfMin,durezaSuperfMax,expCamadaMin,expCamadaMax
    ...
    $pdf->SetFont('Arial','B',9);
    $pdf->Cell(28, 5, number_format($rowMat['durezaNucMin'], 0, ',', '.')." - ".
            number_format($rowMat['durezaNucMax'], 0, ',', '.'),'B,R',0,'L');
    //dureza Superf
    $pdf->SetFont('Arial','',9);
    $pdf->Cell(29, 5, 'Dureza Superficial:','L,B',0,'L');
    $pdf->SetFont('Arial','B',9);
    $pdf->Cell(25, 5, number_format($rowMat['durezaSuperfMin'], 0, ',', '.')." - ".
            number_format($rowMat['durezaSuperfMax'], 0, ',', '.'),'B,R',1,'L');
   ...
   // $pdf->Cell(22, 5, 'Resfriamento','L,B',0,'C');
    ...
    //Inspeção Final
    $pdf->SetFont('Arial','',9);
    $pdf->Cell(34, 5, 'Inspeção Final','L,B,R',1,'C');
    ...
    while($rowIten = $dadosItensOp->fetch(PDO::FETCH_ASSOC)){
       ...
       if ($iK==0){
            //Camada (MM)
            if (($rowMat['expCamadaMin']!='')||($rowMat['expCamadaMax']!='')){
                 $pdf->SetFont('Arial','B',9);
                 $pdf->Cell(22, 10, number_format($rowMat['expCamadaMin'], 0, ',', '.')." - ".
                          number_format($rowMat['expCamadaMax'], 0, ',', '.'),'L,B',0,'C');
            }else{
                 $pdf->Cell(22, 5, ' _   _ ','L',0,'C');
            }
            $iK++;       
       }else {
           $pdf->Cell(22, 5, ' ','L',0,'C');
...
//ETIQUETA DE IDENTIFICAÇÃO DE RETORNO
    ...
    //Dureza Nucleo
    $pdf->SetFont('Arial','',9);
    $pdf->Cell(27, 5, 'Dureza Nuc.Min.:','L,B',0,'L');
    $pdf->SetFont('Arial','B',9);
    $pdf->Cell(27, 5, number_format($rowMat['durezaNucMin'], 0, ',', '.'),'B,R',0,'L');
    
    //Dureza Superf
    $pdf->SetFont('Arial','',9);
    $pdf->Cell(27, 5, 'Dureza Nuc.Max.:','L,B',0,'L');
    $pdf->SetFont('Arial','B',9);
    $pdf->Cell(27, 5, number_format($rowMat['durezaNucMax'], 0, ',', '.'),'B',0,'L');
