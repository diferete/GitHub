    ...
    public function criaTela() {
        ...
        $this->getTela()->setSAcaoShow($sAcaoBusca);
         ...
         $oGridAq->getOGrid()->setSEventoClick('var chave=""; $("#'.$oGridAq->getId().' tbody .selected").each(function(){chave = $(this).find(".chave").html();}); '
                . 'requestAjax("","QualAqEficazApont","sendaDadosCampos","'.$oGridAq->getId().'"+","+chave+","+"'.$oSeqEnv->getId().'"+","+""+","+"'.$oDataPrev->getId().'"+","+"'.$oAcaoEnv->getId().'"+","+"'.$oDataReali->getId().'"+","+"'.$oObs->getId().'"+","+"'.$oEficaz->getId().'"); ');//$oObs
       ...
       //id do grid
        ...
        $sAcaoPlan = 'requestAjax("'.$this->getTela()->getId().'-form","QualAqPlan","geraPlanoEf","'.$this->getTela()->getId().'-form");';
        ...
        $oFieldPlanAcao->setOculto(true);
         ...
         $this->addCampos(array($oEmpresa,$oNr),$oGridAq,array($oSeqEnv,$oDataPrev),
                $oLinha,$oFieldEficaz,
                 $oFieldPlanAcao);
    }
