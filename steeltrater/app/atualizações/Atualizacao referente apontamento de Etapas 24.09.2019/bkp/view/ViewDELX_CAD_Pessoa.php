...
class ViewDELX_CAD_Pessoa extends View {
    ...
    public function criaTela() {
        ...
        $oFieldAdd->addCampos(array($oEmpCob,$oTransp),
                $oRedesp,
        ...
        $oFieldClas->addCampos(array($oClien,$oForne,$oRepr,$oTran,$oAssis,$oFunci),
        ...
        $oFieldObser->addCampos($oObsGer,$oObsCom,
                $oDesAte,$oJusblo,
                $oObsFin,$oObsTra);
        
        $oFieldInf->addCampos(array($oAvaFor,$oDatHom),
                array($oForCon,$oPerFor),
                array($oNotAdQ,$oFrete),
                array($oPerExcQn,$oPerExcUn),
                array($oExiTabPr,$oPadAnf),
                $oCodAce);
        
                
        $oFieldTra->addCampos(array($oRntrc,$oCateg,$oSitRnt,$oUtiTab),
                array($oCarDDR,$oEnvXML));
                
        $oFieldData->addCampos($oDatAlt);
                
        $oAbaGeral->addCampos($oField,
                $oFieldAdd,
                $oFieldClas,
                $oFieldOper,
                $oFieldObser,
