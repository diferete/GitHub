...
foreach ($aOps as $key => $aOp) {
   ...
   //busca itens do tratamento
    ...
    if ($row['retrabalho']=='Sim' || $row['retrabalho']=='Sim S/Cobrança'){
        ...
        $pdf->Cell(110,8,'                  ORDEM DE PRODUÇÃO ',1,0,'L');
        $pdf->Code39(155,$pdf->GetY()+1,$row['op'],1,5);
        $pdf->Cell(52,8,' ','L,B,T,R',1,'C');           
    }
    ...
    if (($row['durezaSuperfMin']!=0)&&($row['durezaSuperfMax']!=0)){
        ...
        $pdf->SetFont('Arial','B',9);
        $pdf->Cell(23, 5, "Min ".number_format($row['durezaSuperfMin'], 0, ',', '.')." - ".
            "Max ".number_format($row['durezaSuperfMax'], 0, ',', '.')."  ".$row['SuperEscala'],'B,R',1,'L');
    }else if ($row['durezaSuperfMax']!=0){
        $pdf->SetFont('Arial','',9);
        $pdf->Cell(28, 5, 'Dureza Superficial:','L,B',0,'L');
        $pdf->SetFont('Arial','B',9);
        $pdf->Cell(23, 5, "Max ".number_format($row['durezaSuperfMax'], 0, ',', '.')."  ".$row['SuperEscala'],'B,R',1,'L');
    }
    else if ($row['durezaSuperfMin']!=0){
        $pdf->SetFont('Arial','',9);
        $pdf->Cell(28, 5, 'Dureza Superficial:','L,B',0,'L');
        $pdf->SetFont('Arial','B',9);
        $pdf->Cell(23, 5, "Min ".number_format($row['durezaSuperfMin'], 0, ',', '.')."  ".$row['SuperEscala'],'B,R',1,'L');
    }else{
    ...
    //
    $pdf->SetFont('Arial','B',9);
    $pdf->Cell(199,5,'TRATAMENTO TÉRMICO',1,1,'C');
    ...
    while($rowIten = $dadosItensOp->fetch(PDO::FETCH_ASSOC)){
       ...
       if ($iK==0){
            ...
            if (($row['expCamadaMin']!='')||($row['expCamadaMax']!='')){
                 $pdf->SetFont('Arial','B',9);
                 $pdf->Cell(30, $iQnt*5, number_format($row['expCamadaMin'], 3, ',', '.')." - ".
                          number_format($row['expCamadaMax'], 3, ',', '.'),'L',0,'C');
            }else{
