...
//busca os dados do banco pegando a op do foreach
    ...
    if ($row['tipoOrdem'] == "P") {
        ...
        if (($row['durezaSuperfMin'] != 0) && ($row['durezaSuperfMax'] != 0)) {
            $pdf->Cell(144, 5, "Min " . number_format($row['durezaSuperfMin'], 0, ',', '.') . " - " .
                    "Max " . number_format($row['durezaSuperfMax'], 0, ',', '.') . "  " . $row['superEscala'], '', 1, 'L');
        ...
        if (($row['certDurezaSuperfMin'] != 0) && ($row['certDurezaSuperfMax'] != 0)) {
            $pdf->Cell(144, 5, "Min " . number_format($row['certDurezaSuperfMin'], 0, ',', '.') . " - " .
                    "Max " . number_format($row['certDurezaSuperfMax'], 0, ',', '.') . "  " . $row['superEscala'], '', 1, 'L');
