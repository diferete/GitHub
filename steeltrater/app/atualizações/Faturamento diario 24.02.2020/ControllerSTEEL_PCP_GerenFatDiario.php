        ...
        $aDadosFat = $this->Persistencia->getFatDiario($aDadosParam['dataini'],$aDadosParam['datafin']);
         ...
         foreach ($aDadosFat['grid'] as $key => $value) {
             ...
             $sHtmRetorno .=  '<td style="font-size:13px;background-color:#D3D3D3">'.$value['dataChar'].' </td>'
                             ...
                             .'<td style="font-size:13px;background-color:#D3D3D3">R$ '.number_format($value['PesoInd'], 2, ',', '.').' </td>'
                             .'<td style="font-size:13px;background-color:#D3D3D3">R$ '.number_format($value['PesoIndAcab'], 2, ',', '.').' </td>'
                             .'<td style="font-size:13px;background-color:#D3D3D3">R$ '.number_format($value['PesoIndFio'], 2, ',', '.').' </td>';
             $sHtmRetorno .='</tr>';
         }
