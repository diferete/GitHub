...
class ViewSTEEL_PCP_OrdensFab extends View {
    ...
    public function RelFaturamento() {
        ...
        //-------------------------------------------------------------------------
        $oTipoMovimento = new Campo('Tipo Movimento', 'nfs_tipomovimentocodigo', Campo::TIPO_SELECTTAGS, 3, 3, 12, 12);
        foreach ($aDados as $key => $oValue) {
            $oTipoMovimento->addItemSelect($key,$oValue);
        }
        ...
        $oField1->addCampos(array($oDatainicial, $oDatafinal),$oLinha1, array($oEmp_codigo, $oEmp_des),$oLinha1, array($oCodigo, $oProdes),$oLinha1);
        
        $oField2 = new FieldSet('Grupos e familias');
        $oField2->setOculto(false);
        $oField2->addCampos(array($oGrupoCod, $oGrupoDes, $oSubGrupoCod, $oSubGrupoDes),$oLinha1,array($oFamiliaCod, $oFamiliaDes, $oSubFamiliaCod, $oSubFamiliaDes));

        $this->addCampos($oField1,$oField2, $oLinha1,$oTipoMovimento);
    }
