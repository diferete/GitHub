...
//----------------------------------------------------------------------------------------------------------
        ...
        if($oDadosOp->getTipoOrdem()=='F'){
                ...
                $this->addCampos($oOpsForno,$oDivApontaOp,array($oTurno,$oFornoChoice,$oCorrida,$oOp,$oBtnInserir),
                $oBtnAtualizar,$oLabelCorrida,
              ...
              if($oDadosOp->getTipoOrdem()=='P'){  
                $oOp->setBFocus(true);
                $this->addCampos($oOpsForno,$oDivApontaOp,array($oTurno,$oFornoChoice,$oOp,$oBtnInserir),
                $oBtnAtualizar,$oLabelCorrida,
                $oGridInicioProcesso,$oGridEnt,/*$oLinha1,$oTurnoFinal,*/$oLinha1,
                $oBotaoFinalizarGeral,/*$oBotaoReabrirGeral,*/
                array($oCodUser,$oUserNome),array($oFornoCod,$oFornoDes));
     ...
     public function criaModalApontaIniciar($aForno, $IdGrid, $sModel) {
       ...
       $sCombo ='var textCombo = $("#'.$oFornoChoice->getId().' option:selected").text(); '
        ...
        if($sModel=='modalApontaIniciarGeren'){
            ...
            $oUserEntcodigo->setBCampoBloqueado(true);
    ...
    public function criaTelaModalApontaFinalizar($aForno,$IdGrid, $sModel) {
       ...
       $sCombo ='var textCombo = $("#'.$oFornoChoice->getId().' option:selected").text(); '
        ...
        $oUserSaicodigo->setBCampoBloqueado(true);
             ...
             //se o tratamento for trefila adiciona os campos diam
                if($oDados->getSTEEL_PCP_Tratamentos()->getTratcod()==20){
                       $this->addCampos(array($oOp,$oEtapa,$oCodEtapa,$oTratDes,$oUserNome),
                        ...
                        $oDiv,
                        $oLinha,
                        array($oTurno,$oDiamMin,$oDiamMax),
                        $oLinha,
                        $oBtnInserir,
                        array($oFornoCod,$oFornoDes,$oCodUser));
                }else{
