...
foreach ($aOps as $key => $aOp) {
    ...
    if ($row['tipoOrdem'] == 'F' || $row['tipoOrdem'] == 'A') {
        $pdf->SetFont('Arial', '', 9);
        $pdf->Cell(19, 5, 'Corrida:', 'L,B', 0, 'L');
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->Cell(30, 5, $rowApont['corrida'], 'B,R', 0, 'C');
    } else {
        $pdf->SetFont('Arial', '', 9);
...
$pdf->Output('I', 'OpSteelTrefila.pdf');
