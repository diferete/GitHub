...
$sNotaFiscal = $_REQUEST['documento'];
}
...
$sSqli = "select NFS_NOTAFISCAL.nfs_notafiscalseq,
                ...
                NFS_NotaFiscalValorDespesasAce
               ...
               "and PRO_GrupoCodigo between '". $sGrupCod ."' and '". $sGrupCodFin ."' ".
               "and PRO_SubGrupoCodigo between '". $sSubGrupCod."' and '". $sSubGrupCodFin ."' ".
               "and PRO_FamiliaCodigo between '". $sFamiliaCod."' and '". $sFamiliaCodFin ."' ".
               "and PRO_SubFamiliaCodigo between '". $sSubFamiliaCod."' and '". $sSubFamiliaCodFin ."' ";
               ...
               //filtro de cliente
                ...
                if(isset($sNotaFiscal)){
                    $sSqli .= " and NFS_NotaFiscalNumero = '".$sNotaFiscal."' ";
                }
...
$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(16, 5, 'Peso Líq.', 'B,R,T', 0, 'C', 0);

$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(16, 5, 'Volumes', 'B,R,T', 0, 'C', 0);

$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(16, 5, 'Peso Op', 'B,R,T', 0, 'C', 0);

$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(18, 5, 'Valor Prod.', 'B,R,T', 0, 'C', 0); 

$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(18, 5, 'Valor Total', 'B,R,T', 1, 'C', 0);
...
while ($row = $dadosRela->fetch(PDO::FETCH_ASSOC)) {
   ...
   // $pdf->Cell(20, 5, $row['NFS_NotaFiscalEmpCNPJ'], 'B,R,T', 0, 'C', 0);
    ...
    $pdf->SetFont('Arial', '', 6);
    $pdf->Cell(16, 5,  number_format($row['NFS_NotaFiscalPesoLiquido'], 2, ',', '.'), 'B,R,T', 0, 'C', 0);
    
    $pdf->SetFont('Arial', '', 6);//NFS_NotaFiscalVolumes 
    $pdf->Cell(16, 5, number_format( $row['NFS_NotaFiscalVolumes'], 2, ',', '.'), 'B,R,T', 0, 'C', 0);

    $pdf->SetFont('Arial', '', 6);//NFS_NotaFiscalVolumes 
    $pdf->Cell(16, 5, number_format( $row['pesoOp'], 2, ',', '.'), 'B,R,T', 0, 'C', 0);

    $pdf->SetFont('Arial', '', 6);
    $pdf->Cell(18, 5, number_format($row['ValorTotalVenda'], 2, ',', '.'), 'B,R,T', 0, 'C', 0);
    
    $pdf->SetFont('Arial', '', 6);
    $pdf->Cell(18, 5, number_format($row['NFS_NotaFiscalValorTotal'], 2, ',', '.'), 'B,R,T', 1, 'C', 0);
    ...
    //total bruto da nota
    $PesoBrutoNota = $PesoBrutoNota + $row['NFS_NotaFiscalPesoBruto'];
    
    //total líquido da nota 
    $PesoLiquido = $PesoLiquido + $row['NFS_NotaFiscalPesoLiquido'];
    
    //Peso da op
    $PesoOp = $PesoOp + $row['pesoOp'];
    
    //valor frete
    $valorFrete =$valorFrete + $row['NFS_NotaFiscalValorFrete'];
    
    //despesa acessória 
    $despAcessorio =$despAcessorio+ $row['NFS_NotaFiscalValorDespesasAce'];
    ...
    if($bIten){
        ...
        $sSqlIten = "select NFS_NOTAFISCAL.nfs_notafiscalseq,
                    ...
                    on NFS_NOTAFISCALITEM.NFS_NotaFiscalFilial = STEEL_PCP_CargaInsumoServ.pdv_pedidofilial
                    and NFS_NOTAFISCALITEM.NFS_NotaFiscalItemPedidoCodigo = STEEL_PCP_CargaInsumoServ.pdv_pedidocodigo
                    and NFS_NOTAFISCALITEM.NFS_NotaFiscalItemPedidoItemSe = STEEL_PCP_CargaInsumoServ.pdv_pedidoitemseq left outer join PRO_PRODUTO 
                      on NFS_NOTAFISCALITEM.NFS_NotaFiscalItemProduto = PRO_PRODUTO.PRO_Codigo 
    ...
    //busca somatórios dos impostos
    $sSqlimpostos = "select SUM(NFS_NotaFiscalAgrupValorIPI) as ipi 
    from NFS_NOTAFISCALAGRUP where NFS_NotaFiscalSeq =".$row['nfs_notafiscalseq'];
    $dadosImpostos = $PDO->query($sSqlimpostos);
    $rowImpostos = $dadosImpostos->fetch(PDO::FETCH_OBJ);
    
    $valorIPI = $valorIPI+ $rowImpostos->ipi;
}
...
if($sSomenteVenda=='Sim'){
 $pdf->Cell(120, 5, 'Valor total com impostos: '.number_format($total_finan+$total_desconto, 2, ',', '.'), 'B', 1, 'L', 0);
 $pdf->Cell(0, 3, '', '', 1, 'L');
 $pdf->Cell(120, 5, 'Valor IPI: '.number_format($valorIPI, 2, ',', '.'), 'B', 1, 'L', 0);
 $pdf->Cell(0, 3, '', '', 1, 'L');
 $pdf->Cell(120, 5, 'Valor FRETE: '.number_format($valorFrete, 2, ',', '.'), 'B', 1, 'L', 0);//$valorFrete
 $pdf->Cell(0, 3, '', '', 1, 'L');
 $pdf->Cell(120, 5, 'Despesa acessório: '.number_format($despAcessorio, 2, ',', '.'), 'B', 1, 'L', 0);
 
 $pdf->Cell(0, 3, '', '', 1, 'L');
 $pdf->Cell(120, 5, 'Descontos aplicados: '.number_format($total_desconto, 2, ',', '.'), 'B', 1, 'L', 0);
 $pdf->Cell(0, 3, '', '', 1, 'L');
 $pdf->Cell(120, 5, 'Valor produtos que são venda: '.number_format($total_produtos, 2, ',', '.'), 'B', 1, 'L', 0);
 $pdf->Cell(0, 3, '', '', 1, 'L');
 $pdf->Cell(120, 5, 'Valor produtos que são venda *C/Desconto *C/IPI: '.number_format($total_produtosDesc+$valorIPI, 2, ',', '.'), 'B', 1, 'L', 0);
 
 $pdf->Cell(0, 3, '', '', 1, 'L');
 
 $pdf->Cell(0, 3, '', '', 1, 'L');
 $pdf->SetFillColor(200,220,255);
 $pdf->Cell(120, 5, 'Detalhamento dos Pesos ', '', 1, 'L', true);
 $pdf->Cell(0, 3, '', '', 1, 'L');
 $pdf->Cell(120, 5, 'Peso bruto da nota fiscal: '.number_format($PesoBrutoNota, 2, ',', '.'), 'B', 1, 'L', 0);
 $pdf->Cell(0, 3, '', '', 1, 'L');
 $pdf->Cell(120, 5, 'Peso líquido da nota fiscal: '.number_format($PesoLiquido, 2, ',', '.'), 'B', 1, 'L', 0);
 $pdf->Cell(0, 3, '', '', 1, 'L');
 $pdf->Cell(120, 5, 'Peso Ops retornadas: '.number_format($PesoOp, 2, ',', '.'), 'B', 1, 'L', 0);
}else{
 $pdf->Cell(120, 5, 'Valor total com impostos: '.number_format($total_finan+$total_desconto, 2, ',', '.'), 'B', 1, 'L', 0);
 $pdf->Cell(0, 3, '', '', 1, 'L');
 $pdf->Cell(120, 5, 'Valor IPI: '.number_format($valorIPI, 2, ',', '.'), 'B', 1, 'L', 0);
 ...
 $pdf->Cell(0, 3, '', '', 1, 'L');
 $pdf->Cell(120, 5, 'Valor Produtos *C/Desconto *C/IPI: '.number_format($total_produtosDesc+$valorIPI, 2, ',', '.'), 'B', 1, 'L', 0);
