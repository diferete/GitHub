...
foreach ($nCargas as $key => $aCarga) {
    ...
    foreach ($aOps as $key => $aOp) {
        if ($aOp != null) {
            ...
            $pdf->Cell(54, 4, substr($useRel, 0, 20), 'L,B,R', 1, 'L');
            $pdf->Cell(19, 4, 'Dt.: ' . $data, 'L,B,R', 0, 'L');
            $pdf->Cell(16, 4, 'Hr: ' . $hora, 'L,B,R', 0, 'L');
            ...
            //Cliente
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->MultiAlignCell(54, 4, 'Cliente: ' . $row['emp_razaosocial'], 1, 1, 'L', false);

            //Data de entrada
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->Cell(15, 4, 'Dt. entrada:', 'L,B', 0, 'L');
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->Cell(16, 4, $row['data'], 'B,R', 0, 'L');

            //Número da OP
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->Cell(11, 4, 'Nº OP:', 'L,B', 0, 'L');
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->Cell(12, 4, $row['op'], 'B,R', 1, 'L');

            //OP do cliente
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->Cell(14, 4, 'OP Cliente:', 'L,B', 0, 'L');
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->Cell(40, 4, $row['opcliente'], 'B,R', 1, 'L');

            //Peso
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->Cell(8, 4, 'Peso:', 'B,L', 0, 'L');
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->Cell(15, 4, number_format($row['peso'], 2, ',', '.'), 'B,R', 0, 'L');

            //Material
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->Cell(12, 4, 'Material:', 'B,L', 0, 'L');
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->Cell(19, 4, $rowMat['matdes'], 'B,R', 1, 'L');

//        //Dureza Nucleo
//        $pdf->SetFont('Arial', 'B', 7);
//        $pdf->Cell(20, 4, 'Dureza Núcleo:', 'L,B', 0, 'L');
//        $pdf->SetFont('Arial', 'B', 7);
//        $pdf->Cell(34, 4, number_format($row['durezaNucMin'], 2, ',', '.') . " - " .
//                number_format($row['durezaNucMax'], 2, ',', '.') . "  " . $row['NucEscala'], 'B,R', 1, 'L');
//
//        //Dureza Superf
//        if (($row['durezaSuperfMin'] != 0) && ($row['durezaSuperfMax'] != 0)) {
//            $pdf->SetFont('Arial', 'B', 7);
//            $pdf->Cell(54, 4, 'Dureza Superficial:'. number_format($row['durezaSuperfMin'], 2, ',', '.') . " - " .
//                    number_format($row['durezaSuperfMax'], 2, ',', '.') . "  " . $row['SuperEscala'], 'B,R,L', 1, 'L');
//        } else if ($row['durezaSuperfMax'] != 0) {
//            $pdf->SetFont('Arial', 'B', 7);
//            $pdf->Cell(54, 4, "Dureza Superficial:"." Max " . number_format($row['durezaSuperfMax'], 2, ',', '.') . "  " . $row['SuperEscala'], 'B,R,L', 1, 'L');
//        } else if ($row['durezaSuperfMin'] != 0) {
//            $pdf->SetFont('Arial', 'B', 7);
//            $pdf->Cell(54, 4, "Dureza Superficial:"." Min " . number_format($row['durezaSuperfMin'], 2, ',', '.') . "  " . $row['SuperEscala'], 'B,R,L', 1, 'L');
//        }
            ...
            $pdf->Cell(20, 4, 'Dureza Núcleo:', 'L,B', 0, 'L');
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->Cell(34, 4, number_format($rowCert['durezanucmin'], 2, ',', '.') . " - " .
                    number_format($rowCert['durezanucmax'], 2, ',', '.') . "  " . $rowCert['nucescala'], 'B,R', 1, 'L');

            //Produto
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->MultiAlignCell(54, 4, 'Produto: ' . $row['referencia'] . " - " . $row['prodes'], 1, 1, 'L');
