...
foreach ($aOps as $key => $aOp) {
    ...
    $sSql = "select op, 
            ...
            emp_razaosocial,fioDurezaSol,fioEsferio,
    ...
    if ($row['retrabalho'] == 'Sim' || $row['retrabalho'] == 'Sim S/Cobrança') {
        ...
        $pdf->Cell(74, 8, 'OP - RETRABALHO - TREFILA', 1, 1, 'C');
        $pdf->Code39(10, $pdf->GetY() + 1, $row['op'], 1.4, 7);
        $pdf->Cell(74, 9, ' ', 'L,B,T,R', 1, 'C');
    } else {
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(74, 8, 'ORDEM DE PRODUÇÃO - TREFILA', 1, 1, 'C');
        $pdf->Code39(10, $pdf->GetY() + 1, $row['op'], 1.4, 7);
        $pdf->Cell(74, 9, ' ', 'L,B,T,R', 1, 'C');
    }
    ...
    if ($row['tipoOrdem'] == 'F' || $row['tipoOrdem'] == 'A') {
        //op do cliente
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->Cell(22, 5, 'Corrida:', 'L,B', 0, 'L');
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->Cell(15, 5, $rowApont['corrida'], 'B,R', 1, 'L');
    } else {
        //op do cliente
        $pdf->SetFont('Arial', 'B', 9);
    ...
    //produto
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->MultiAlignCell(74, 5, 'Produto: '.$row['referencia'] . " - " . $row['prodes'], 'L,B,R', 1, 'L');
    ...
    $pdf->SetFont('Arial','B',8);
    $pdf->MultiAlignCell(74, 5, 'Produto Final: '.$row['prodFinal'] . " - " . $row['prodesFinal'], 'L,B,R', 1, 'L');
    ///////////////////
    
    $pdf->SetFont('Arial','B',8);
    $pdf->Cell(22, 5, 'Dureza (HRB):','L,B',0,'L');
    $pdf->SetFont('Arial','B',8);
    $pdf->Cell(13, 5, number_format($row['fioDurezaSol'], 0, ',', '.'),'B,R',0,'L');
    
    $pdf->SetFont('Arial','B',8);
    $pdf->Cell(16, 5, 'Esferoidização:','L,B',0,'L');
    $pdf->SetFont('Arial','B',8);
    $pdf->Cell(23, 5, number_format($row['fioEsferio'], 0, ',', '.').'%','B,R',1,'C');
    
    $pdf->SetFont('Arial','B',8);
    $pdf->Cell(15, 5, 'Desc.(µm):','L,B',0,'L');
    $pdf->SetFont('Arial','B',8);
    $pdf->Cell(20, 5, number_format($row['fioDescarbonetaParcial'], 0, ',', '.').' - '
            .number_format($row['fioDescarbonetaTotal'], 0, ',', '.').'','B,R',0,'C');
    
    $pdf->SetFont('Arial','B',8);
    $pdf->Cell(21, 5, 'Diâm.Fin.(µm)','L,B,R',0,'L');
    $pdf->SetFont('Arial','B',8);
    $pdf->Cell(9, 5, number_format($row['DiamFinalMin'], 2, ',', '.'),'B,R',0,'C');
    $pdf->Cell(9, 5, number_format($row['DiamFinalMax'], 2, ',', '.'),'B,R',1,'C');
    
        ////////////
    
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->Cell(74, 5, 'TRATAMENTO TÉRMICO', 1, 1, 'C');
...
$pdf->Output('I', 'OpEtiquetaTrefila.pdf');
