...
class PDF extends FPDF {
    ...
    function Footer() { // Cria rodapé
        ...
        $this->Image('../../biblioteca/assets/images/steelrel.png', 180, 286, 20);
    }
...
$pdf->Image('../../biblioteca/assets/images/steelrel.png', 3, 9, 40); // INSERE UMA LOGOMARCA NO PONTO X = 11, Y = 11, E DE TAMANHO 40.
...
$sql = "select nr, filcgc, fil_fantasia, titulo, convert(varchar,dtimp,103) as dtimp, userimp, convert(varchar,dataini,103) as dataini, datafim, tipoacao, origem, sit, descsetor, equipe from MET_QUAL_qualaq
        left outer join FIL_EMPRESA 
        on MET_QUAL_qualaq.filcgc = FIL_EMPRESA.fil_codigo
        where filcgc = '8993358000174' and FIL_Fantasia = 'STEELTRATER'";
...
while ($row = $sth->fetch(PDO::FETCH_ASSOC)) {
    ...
    if ($sVencido != 'true') {
        ...
        $pdf->SetFont('Arial', '', 7);
        $pdf->Cell(95, 5, $row['filcgc'] . ' - ' . $row['fil_fantasia'], 'B,R,T', 0, 'L');
    ...
    $sql2 = "select plano,seq, convert(varchar,dataprev,103) as dataprev,"
            ...
            . " from MET_QUAL_qualplan where nr =" . $row['nr'] . " and filcgc = '8993358000174' ";
    if ($sSituacao2 == 'F') {
