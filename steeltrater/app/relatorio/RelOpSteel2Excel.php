...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('G4', 'Cod. Tratamentos')
        ->setCellValue('H4', "$sTratCod")
...
while ($row = $dadosRela->fetch(PDO::FETCH_ASSOC)) {
    ...
    $situac = $row['situacao'];
    $iPesototal = (float)$iPesototal + (float)$iPeso2;
    $iQuantidadetotal = (float)$iQuantidadetotal + (float)$iQuant;
    $objPHPExcel->setActiveSheetIndex(0)
