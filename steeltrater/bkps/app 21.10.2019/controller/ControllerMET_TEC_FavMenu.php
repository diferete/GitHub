    ...
    public function getFavMenu($bRecarrega=null){
       ...
       $sString='';
        foreach ($aFav as $key => $avalue) {
          ...
          .'                  <span class="icon fa-star-o"></span>'.$avalue[1].'<span title="Deleta favorito" onclick="requestAjax(\\\'menu-'.$sMenuId.'\\\',\\\'MET_TEC_FavMenu\\\',\\\'msgdeletaFav\\\',\\\''.utf8_encode($avalue[2]).','.utf8_encode($avalue[3]).',\\\');" class="icon wb-trash pull-right"></span></a>'
          .'              </li>';
        }
       }else{
          foreach ($aFav as $key => $avalue) {
