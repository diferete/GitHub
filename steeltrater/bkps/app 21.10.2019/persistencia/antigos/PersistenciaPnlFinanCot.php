...
class PersistenciaPnlFinanCot extends Persistencia{
    ...
    public function consultaManual() {
       ...
       widl.REC0012.recprnro as 'widl.REC001.recprnro'   
        ...
        /*where widl.REC001.empcod ='44170801000170'
    ...
    public function somaTitAtraso(){
         $sSql = "select SUM(recprvlr)as total 
            ...
            and widl.REC001.filcgc = '75483040000211'
            and recprdtpgt = '1753-01-01'
            and tpdcod in (1,3) and DATEDIFF(day,CONVERT (date, SYSDATETIME()),recprdtpro) < 0 ";
    ...
    public function mediaFat($sEmpcod){
        ...
        $sSql = "select  SUM(nfsitvlrto + nfsitvlrip + nfsitvlrsu)/12 total 
                        ...
                        and nfsdtemiss between convert(date,'".$sDataIni."') and CONVERT (date, SYSDATETIME()) 
                        and nfscancela <> '*'
    ...
    public function limiteCred($sEmpcod){
        $sSql = "select empcod,empdes,cclilimite,repcod from widl.emp01 where empcod =".$sEmpcod;
        $result = $this->getObjetoSql($sSql);
        $row = $result->fetch(PDO::FETCH_OBJ);
