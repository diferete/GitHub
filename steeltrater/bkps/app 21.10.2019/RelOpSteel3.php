...
foreach ($aOps as $key => $aOp) {
    //Quebra pagina após três receitas
    if(($icont > 3)||($iop>1)){
         $pdf->AddPage();
         $pdf->SetXY(5,5);
         $icont=0;
   ...
   //busca itens do tratamento
    ...
    if ($row['retrabalho']=='Sim'){
        $pdf->Cell(102,10,'ORDEM DE PRODUÇÃO - RETRABALHO',1,0,'C',0);
    }else{
        $pdf->Cell(102,10,'ORDEM DE PRODUÇÃO',1,0,'C',0);
    }   
    ...
    if ($row['retrabalho']=='Sim'){
        ...
        $pdf->SetFont('Arial','',9);
        $pdf->Cell(8, 5, 'Nº:','L,B',0,'L');
        $pdf->SetFont('Arial','B',9);
        $pdf->Cell(17, 5, $row['op'],'B,R',0,'C');
             
        $pdf->SetFont('Arial','',9);
        $pdf->Cell(18, 5, 'OP Origem:','B',0,'C');
        $pdf->SetFont('Arial','B',9);
        $pdf->Cell(17, 5, $row['op_retrabalho'],'B,R',1,'C');
