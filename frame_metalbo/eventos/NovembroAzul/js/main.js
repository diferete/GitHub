...
function getDadosFunc() {
    var cracha = $('#cracha').val();
    if (cracha === '' || cracha === null) {
        $("#waiting-msg").text('Oops, digite o seu crachá!').addClass('email-error').removeClass('shake animated hidden email-success').addClass('shake animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
            $("#waiting-msg").removeClass('shake animated');
        });
    } else {
        $.getJSON('http://localhost/GitHub/frame_metalbo/index.php?classe=MET_RH_Colaboradores&metodo=getDadosFunc' + '&dados=' + cracha, function (result) {
            gerenSituacao(result);
        });
    }
}

function gerenSituacao(result) {
    if (result.dados[0] !== 'false') {
        $("#dadosHidden").show();
        $("#waiting-msg").removeAttr('class').addClass('msg-email hidden');
        $("#nomeFun").val(result.dados.nome).prop('readonly', true);
        $("#setorFun").val(result.dados.setor).prop('readonly', true);
        $("#tipsex").text(result.dados.tipsex);

        $("#waiting-msg").show().text('Confira seus dados e clique no botão abaixo para confirmar seu cadastro!').addClass('email-success').removeClass('shake animated hidden email-error').addClass('shake animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
            $("#waiting-msg").removeClass('shake animated');
        });
    } else {
        var situaca = result.dados[1];
        switch (situaca) {
            case 'E':
                $("#waiting-msg").show().text('Verifique o número do crachá digitado!').addClass('email-error').removeClass('shake animated hidden email-success').addClass('shake animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
                    $("#waiting-msg").removeClass('shake animated');
                });
                $("#dadosHidden").hide();
                break;
            case 'C':
                $("#waiting-msg").show().text('Você já possui cadastro, porém pode assistir o vídeo novamente e confirmar que assistiu no final. Lembre-se que o sorteio só vale para quem cadastrou o crachá.').removeClass('email-error').addClass('email-warning').removeClass('shake animated hidden email-success').addClass('shake animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
                    $("#waiting-msg").removeClass('shake animated');
                    $('#waiting-msg').append('<span class="back"><span>C</span><span>a</span><span>r</span><span>r</span><span>e</span><span>g</span><span>a</span><span>n</span><span>d</span><span>o</span></span>');
                });
                setTimeout(function () {
                    $("#waiting-msg").hide();
                    $("#send-form").hide();
...
function gravaDadosFunc() {
    var cracha = $('#cracha').val();
    var tipsex = $('#tipsex').text();
    $.getJSON('http://localhost/GitHub/frame_metalbo/index.php?classe=MET_RH_Colaboradores&metodo=gravaDadosFunc' + '&dados=' + cracha, function (result) {
        if (result) {
            if (tipsex === 'F') {
                $("#send-form").hide();
                $("#waiting-msg-cad").show().text('Cadastro efetuado com sucesso. Assita ao vídeo para completar o cadastro!').addClass('email-success').removeClass('shake animated hidden email-error').addClass('shake animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
                    $("#waiting-msg-cad").removeClass('shake animated');
                });
                $('#waiting-msg-cad').append('<span class="back"><span>C</span><span>a</span><span>r</span><span>r</span><span>e</span><span>g</span><span>a</span><span>n</span><span>d</span><span>o</span></span>');
                setTimeout(function () {
                    $("#waiting-msg-cad").hide();
                    $("#video").show();
                    $("#vid").get(0).play();
                }, 5000);
            } else {
                $("#send-form").hide();
                $("#waiting-msg-cad").show().text('Cadastro efetuado com sucesso. Assista ao vídeo até o final para concorrer ao BRINDE!').addClass('email-success').removeClass('shake animated hidden email-error').addClass('shake animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
                    $("#waiting-msg-cad").removeClass('shake animated');
                });
                $('#waiting-msg-cad').append('<span class="back"><span>C</span><span>a</span><span>r</span><span>r</span><span>e</span><span>g</span><span>a</span><span>n</span><span>d</span><span>o</span></span>');
                setTimeout(function () {
            ...
            $("#waiting-msg").text('Hmm, parece que temos um problema ao tentar cadastrar, entre em contato com o TI da Metalbo!').addClass('email-error').removeClass('shake animated hidden email-success').addClass('shake animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
                $("#waiting-msg").removeClass('shake animated');
            });
        }
    });
...
$('#vid').on('ended', function () {
    var tipsex = $('#tipsex').text();
    if (tipsex === 'F') {
        var result = confirm("Você assistiu o vídeo completo! Clique em OK para completar o seu cadastro :).");
        if (result) {
            if (result) {
                $("#video").hide();
                $("#send-form").show();
        ...
        var result = confirm("Você assistiu o vídeo completo! Clique OK para garantir uma vaga no sorteio e boa sorte :).");
        if (result) {
            var cracha = $('#cracha').val();
            $.getJSON('http://localhost/GitHub/frame_metalbo/index.php?classe=MET_RH_Colaboradores&metodo=updateDadosFunc' + '&dados=' + cracha, function (result) {
                if (result) {
                    $("#video").hide();
                    $("#send-form").show();
                    $("#cracha").val('').focus();
