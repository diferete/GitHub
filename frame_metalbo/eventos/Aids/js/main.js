    ...
    if (result.dados[0] !== 'false') {
        ...
        switch (situaca) {
            case 'E':
                ...
                break;
            case 'C':
                $("#waiting-msg").show().text('Você já possui cadastro, porém pode assistir o vídeo novamente e confirmar que assistiu no final.').removeClass('email-error').addClass('email-warning').removeClass('shake animated hidden email-success').addClass('shake animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
                    $("#waiting-msg").removeClass('shake animated');
...
function gravaDadosFunc() {
    var cracha = $('#cracha').val();
    $.getJSON('http://localhost/GitHub/frame_metalbo/index.php?classe=MET_RH_Colaboradores&metodo=gravaDadosFuncAids' + '&dados=' + cracha, function (result) {
        if (result) {

            $("#send-form").hide();
            $("#waiting-msg-cad").show().text('Cadastro efetuado com sucesso. Agora você pode assistir ao vídeo!').addClass('email-success').removeClass('shake animated hidden email-error').addClass('shake animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
...
$('#vid').on('ended', function () {
    ...
    var result = confirm("Você assistiu o vídeo completo! Obrigado por participar. :).");
    if (result) {
        var cracha = $('#cracha').val();
        $.getJSON('http://localhost/GitHub/frame_metalbo/index.php?classe=MET_RH_Colaboradores&metodo=updateDadosFuncAids' + '&dados=' + cracha, function (result) {
            if (result) {
                $("#video").hide();
                $("#send-form").show();
                $("#cracha").val('').focus();
