...
class Danfe extends Common {
     ...
     */
    public function monta($aDadosExtras, $logo = '', $orientacao = '', $papel = 'A4', $logoAlign = 'C', $depecNumReg = '', $margSup = 5, $margEsq = 5, $margInf = 5
    ) {
        $this->pdf = '';
        $this->logomarca = $logo;
        ...
        if (isset($this->infAdic)) {
            ...
            $this->textoAdic .= !empty($this->getTagValue($this->infAdic, "infCpl")) ? 'Inf. Contribuinte: ' . $this->anfaveaDANFE($this->getTagValue($this->infAdic, "infCpl")) : '';
        ...
        $y = $this->destinatarioDANFE($x, $y + 1, $aDadosExtras);
        ...
        $y = $this->transporte($x, $y + 1, $aDadosExtras);
        //itens da DANFE
        $nInicial = 0;
    ...
    protected function statusNFe() {
        ...
        if ($cStat == '101' || $cStat == '151' || $cStat == '135' || $cStat == '155'
        ) {
    ...
    protected function header($x = 0, $y = 0, $pag = '1', $totPag = '1') {
        ...
        $y1 = $y + 20;
        $numNF = str_pad(
                $this->ide->getElementsByTagName('nNF')->item(0)->nodeValue, 9, "0", STR_PAD_LEFT
        );
        ...
        if (($this->tpEmis == 2 || $this->tpEmis == 5) && !$this->notaDpec()) {
            ...
            $this->pdf->textBox(
                    $x + 2, $y1, $w - 2, $h, $texto, $aFont, 'T', 'C', 0, 'http://www.nfe.fazenda.gov.br/portal ou no site da Sefaz Autorizadora'
            );
        }
...
//        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'B', 'C', 0, '');
        ...
        if (!empty($this->emit->getElementsByTagName("CNPJ")->item(0)->nodeValue)) {
            ...
            $texto = !empty($this->emit->getElementsByTagName("CPF")->item(0)->nodeValue) ? $this->formatField(
...
//fim header
     ...
     * @return number Posição vertical final
     */
    protected function destinatarioDANFE($x = 0, $y = 0, $aDadosExtras) {
        //####################################################################################
        ...
        if (!empty($this->dest->getElementsByTagName("CNPJ")->item(0)->nodeValue)) {
            ...
            $texto = !empty($this->dest->getElementsByTagName("CPF")->item(0)->nodeValue) ? $this->formatField(
        ...
        $aFont = ['font' => $this->fontePadrao, 'size' => 6, 'style' => ''];
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 1, '');
        $dSaiEnt = $aDadosExtras['dataSaida'];
        ...
        $aFont = ['font' => $this->fontePadrao, 'size' => 6, 'style' => ''];
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 1, '');
        $hSaiEnt = $aDadosExtras['horaSaida'];
...
//fim da função localRetiradaDANFE
    ...
    protected function fatura($x, $y) {
        ...
        if ($this->dup->length > 0 || $textoFatura !== "") {
            ...
            foreach ($this->dup as $k => $d) {
                ...
                $dDup = !empty($this->dup->item($k)->getElementsByTagName('dVenc')->item(0)->nodeValue) ? $this->ymdTodmy($this->dup->item($k)->getElementsByTagName('dVenc')->item(0)->nodeValue) : '';
                $vDup = !empty($this->dup->item($k)->getElementsByTagName('vDup')->item(0)->nodeValue) ? 'R$ ' . number_format(
                                $this->dup->item($k)->getElementsByTagName('vDup')->item(0)->nodeValue, 2, ",", "."
                        ) : '';
                $h = 8;
                $texto = '';
                if ($nDup != '0' && $nDup != '') {
    ...
    protected function pagamento($x, $y) {
        ...
        if (isset($this->detPag) && $this->detPag->length > 0) {
            ...
            foreach ($this->detPag as $k => $d) {
                $fPag = !empty($this->detPag->item($k)->getElementsByTagName('tPag')->item(0)->nodeValue) ? $this->detPag->item($k)->getElementsByTagName('tPag')->item(0)->nodeValue : '0';
                $vPag = !empty($this->detPag->item($k)->getElementsByTagName('vPag')->item(0)->nodeValue) ? 'R$ ' . number_format(
...
//fim da função pagamento
    ...
    protected function imposto($x, $y) {
        ...
        if ($this->exibirIcmsInterestadual) {
            ...
            $x = $this->impostoHelper($x, $y, $w, $h, "VALOR DO FCP", "vFCPUFDest");
        }
...
//fim imposto
     ...
     */
    protected function transporte($x, $y, $aDadosExtras) {
        $oldX = $x;
        if ($this->orientacao == 'P') {
            $maxW = $this->wPrint;
        } else {
        ...
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 1, '');
        if (isset($this->transporta)) {
            $texto = !empty($this->transporta->getElementsByTagName("CNPJ")->item(0)->nodeValue) ? $this->formatField(
                            $this->transporta->getElementsByTagName("CNPJ")->item(0)->nodeValue, "##.###.###/####-##"
                    ) : '';
            if ($texto == '') {
                $texto = !empty($this->transporta->getElementsByTagName("CPF")->item(0)->nodeValue) ? $this->formatField(
                                $this->transporta->getElementsByTagName("CPF")->item(0)->nodeValue, "###.###.###-##"
                        ) : '';
            }
        } else {
            $texto = '00.000.000/0000-00';
        }
        $aFont = ['font' => $this->fontePadrao, 'size' => 10, 'style' => 'B'];
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'B', 'C', 0, '');
        //#####################################################################
        ...
        if ($volumes->length > 0) {
            ...
            $quantidade = $aDadosExtras['nfsqtdvol'];
            $especie = $aDadosExtras['nfsespecie'];
            $marca = $aDadosExtras['nfsmarca'];
        }
        ...
        $texto = !empty($this->transp->getElementsByTagName("marca")->item(0)->nodeValue) ?
                $this->transp->getElementsByTagName("marca")->item(0)->nodeValue : $marca;
        $aFont = ['font' => $this->fontePadrao, 'size' => 10, 'style' => 'B'];
        $this->pdf->textBox($x, $y, $w2, $h, $texto, $aFont, 'B', 'C', 0, '');
        //NUMERAÇÃO
        $x += $w2;
...
//fim transporte
    ...
    protected function descricaoProduto($itemProd) {
        ...
        $infAdProd = !empty($itemProd->getElementsByTagName('infAdProd')->item(0)->nodeValue) ? substr(
                        $this->anfaveaDANFE($itemProd->getElementsByTagName('infAdProd')->item(0)->nodeValue), 0, 500
                ) : '';
        if (!empty($infAdProd)) {
    ...
    protected function itens($x, $y, &$nInicio, $hmax, $pag = 0, $totpag = 0, $hCabecItens = 7) {
        ...
        foreach ($this->det as $d) {
            if ($i >= $nInicio) {
                ...
                $x += $w7;
                // Valor Unitário
                $texto = number_format($prod->getElementsByTagName("vUnCom")->item(0)->nodeValue, 2, ",", ".");
                ...
                $x += $w9;
                if (isset($ICMS)) {
                    $texto = !empty($ICMS->getElementsByTagName("vBC")->item(0)->nodeValue) ? number_format(
                                    $ICMS->getElementsByTagName("vBC")->item(0)->nodeValue, 2, ",", "."
                            ) : '0,00';
                ...
                $x += $w10;
                if (isset($ICMS)) {
                    $texto = !empty($ICMS->getElementsByTagName("vICMS")->item(0)->nodeValue) ? number_format(
                                    $ICMS->getElementsByTagName("vICMS")->item(0)->nodeValue, 2, ",", "."
                            ) : '0,00';
                ...
                $x += $w11;
                if (isset($IPI)) {
                    $texto = !empty($IPI->getElementsByTagName("vIPI")->item(0)->nodeValue) ? number_format(
                                    $IPI->getElementsByTagName("vIPI")->item(0)->nodeValue, 2, ",", "."
                            ) : '0,00';
                } else {
                    $texto = '0,00';
                }
                ...
                $x += $w12;
                if (isset($ICMS)) {
                    $texto = !empty($ICMS->getElementsByTagName("pICMS")->item(0)->nodeValue) ? number_format(
                                    $ICMS->getElementsByTagName("pICMS")->item(0)->nodeValue, 2, ",", "."
                            ) : '0,00';
                ...
                $x += $w13;
                if (isset($IPI)) {
                    $texto = !empty($IPI->getElementsByTagName("pIPI")->item(0)->nodeValue) ? number_format(
                                    $IPI->getElementsByTagName("pIPI")->item(0)->nodeValue, 2, ",", "."
                            ) : '0,00';
                } else {
                    $texto = '';
                }
