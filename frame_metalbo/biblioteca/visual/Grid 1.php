...
class Grid{
    ...
    *
    */    
    function __construct($sTelaGrande = '12',$sTelaMedia = '12',$sTelaPequena = '12',$sTelaMuitoPequena='12',$iAltura=400) {
        $this->sId = Base::getId();
        $this->aColunas = array();
        $this->aBotoes= array();
    ...
    public function getRender(){
      ...
      //foreach a botao e vai montar o html dos botões
       ...
       if(!empty($this->aFiltro)){
               $sFiltro =   ' <div class="row" id="'.$this->getSId().'-filtros" style="display: none; background-color: whitesmoke">'
                            .'<form class="col-lg-12 col-md-12 col-sm-12 col-xs-12" id="'.$this->getSId().'-pesquisa" style=" position: relative; padding: 15px 15px 0px 50px;  background-color: #f3f7fa;  border: 1px solid #eee">'
            ...
            $sFiltro .= '</form>'
                    .'<script>'
                        .'$("#'.$this->getSId().'-pesquisa :input").keyup(function(){'
                            .'var val = $(this).val();'
                          //  .'alert(val);'
       ...
       $sGrid .= '<table id="'.$this->getSId().'" class="display compact" cellspacing="0" width="100%" style="background-color:#E8E8E8" >'//display compact
     ...
     $sBotCarregar ='<button type="button" id="'.$this->getSId().'-botcarr" class="btn btn-primary btn-xs '
      ...
      $sGrid .='<tbody id="'.$this->getSId().'body">'.$sDados.'</tbody></table>'.$sBotCarregar
      .'<div class="panel"><table id="'.$this->getSId().'-summary" class="table table-hover"><tbody><tr class="tr-destaque">';
      $sGrid .=$oDados->getDadosFoot($this->getArrayCampos(), $this->getBGridCampo(), $this->getAParametros()); 
      $sGrid.='</tr></tbody></table></div>';
