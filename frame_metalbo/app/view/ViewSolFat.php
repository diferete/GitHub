...
class ViewSolFat extends View {
    ...
    public function criaConsulta() {
        ...
        $oFatod->addComparacao('0', CampoConsulta::COMPARACAO_MAIOR, CampoConsulta::COR_AMARELO, CampoConsulta::MODO_COLUNA, false, null);
        $oFatod->setBComparacaoColuna(true);

        $oFatsit->addComparacao('Não gerada', CampoConsulta::COMPARACAO_IGUAL, CampoConsulta::COR_VERMELHO, CampoConsulta::MODO_COLUNA, false, null); //Ordem de serviço
        $oFatsit->addComparacao('Faturada', CampoConsulta::COMPARACAO_IGUAL, CampoConsulta::COR_VERDE, CampoConsulta::MODO_COLUNA, false, null);
        ...
        $oFatfinan->addComparacao('Com financeiro', CampoConsulta::COMPARACAO_IGUAL, CampoConsulta::COR_VERDE, CampoConsulta::MODO_COLUNA, false, null);
        $oFatfinan->addComparacao('Sem financeiro', CampoConsulta::COMPARACAO_IGUAL, CampoConsulta::COR_VERMELHO, CampoConsulta::MODO_COLUNA, false, null);
        $oFatfinan->setBComparacaoColuna(true);

        $this->addCampos($oFatnro, $oFatod, $oCnpj, $oCliente, $oContato, $oVlr, $oFatsit, $oFatfinan);


        $oFatSolFil = new Filtro($oFatnro, Filtro::CAMPO_TEXTO, 2, 2, 12, 12, false);

        $oClienteFil = new Filtro($oCliente, Filtro::CAMPO_TEXTO, 3, 3, 12, 12, false);

        $this->addFiltro($oFatSolFil, $oClienteFil);

        $this->setUsaDropdown(true);
        $oDrop1 = new Dropdown('Contas Receber ', Dropdown::TIPO_PADRAO);
        $oDrop1->addItemDropdown($this->addIcone(Base::ICON_CONFIG) . 'Adiciona Contas á Receber', 'ContRec', 'acaoMostraTelaIncluir', '', true, '', false, '', false, '', false, false);

        $oDrop2 = new Dropdown('Movimentações ', Dropdown::TIPO_ERRO);
        $oDrop2->addItemDropdown($this->addIcone(Base::ICON_CONFIG) . 'Retorna solicitação', 'SolFat', 'retornaSolicitação', '', false, '', false, '', false, '', false, false);
