...
class ViewMET_MP_ItensManPrev extends View {
    ...
    function criaGridDetalhe() {
        ...
        $oBotaoFinalizar->addAcao('MET_MP_ItensManPrev', 'msgFinalizaServ', '', '');
        ...
        $oCodSit = new CampoConsulta('Cod.Serviço', 'MET_MP_ServicoMaquina.codsit');
        ...
        $oSitmp->addComparacao('ABERTO', CampoConsulta::COMPARACAO_IGUAL, CampoConsulta::COR_VERDE, CampoConsulta::MODO_COLUNA, false, null);
        ...
        $this->getOGridDetalhe()->setSEventoClick('var chave=""; $("#' . $this->getOGridDetalhe()->getSId() . ' tbody .selected").each(function(){chave = $(this).find(".chave").html();}); '
                . 'requestAjax("","MET_MP_ItensManPrev","camposGrid","' . $this->getOGridDetalhe()->getSId() . '"+","+chave+","+"' . $oServ->getId() . '"+","+"' . $oQueFazer->getId() . '");');
    ...
    public function criaConsulta() {
        ...
        $oServ = new Campo('Serviço', 'MET_MP_ServicoMaquina.servico', Campo::TIPO_TEXTO, 5);
        ...
        $oDias = new CampoConsulta('Dias Restantes', 'dias');
        $oDias->addComparacao('0', CampoConsulta::COMPARACAO_MENOR, CampoConsulta::COR_VERMELHO, CampoConsulta::MODO_COLUNA, false, null);
        ...
        $this->getTela()->setSEventoClick('var chave=""; $("#' . $this->getTela()->getSId() . ' tbody .selected").each(function(){chave = $(this).find(".chave").html();}); '
                . 'requestAjax("","MET_MP_ItensManPrev","camposGrid","' . $this->getTela()->getSId() . '"+","+chave+","+"' . $oServ->getId() . '");');
    ...
    public function criaTela() {
        ...
        $oServ->setClasseBusca('MET_MP_ServicoMaquina');
        ...
        $oResponsavel = new Campo('Responsável', 'MET_MP_ServicoMaquina.resp', Campo::TIPO_TEXTO, 3, 3, 12, 12);
        $oResponsavel->setBCampoBloqueado(true);

        $sCallBack = 'requestAjax("' . $this->getTela()->getId() . '-form","MET_MP_ItensManPrev","buscaDados","' . $oDias->getId() . ',' . $oResponsavel->getId() . '");';
