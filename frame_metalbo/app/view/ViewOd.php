...
class ViewOd extends View {
    ...
    public function criaConsulta() {
        ...
        $oContatoFiltro = new Filtro($oContato, Campo::TIPO_TEXTO, 3, 3, 12, 12, false);
        $oOdNr = new Filtro($oOdnrj, Campo::TIPO_TEXTO, 2, 2, 12, 12, false);

        $oOdSit->addComparacao('Aberta', CampoConsulta::COMPARACAO_IGUAL, CampoConsulta::COR_AZUL, CampoConsulta::MODO_COLUNA, false, null);
        $oOdSit->addComparacao('Encerrada sem Faturamento', CampoConsulta::COMPARACAO_IGUAL, CampoConsulta::COR_AMARELO, CampoConsulta::MODO_COLUNA, false, null);
        $oOdSit->addComparacao('Lib. Faturamento', CampoConsulta::COMPARACAO_IGUAL, CampoConsulta::COR_VERDE, CampoConsulta::MODO_COLUNA, false, null);
        $oOdSit->setBComparacaoColuna(true);

        $oOdTipo->addComparacao('Pedido de venda', CampoConsulta::COMPARACAO_IGUAL, CampoConsulta::COR_VERDE, CampoConsulta::MODO_COLUNA, false, null); //Ordem de serviço
        $oOdTipo->addComparacao('Ordem de serviço', CampoConsulta::COMPARACAO_IGUAL, CampoConsulta::COR_AZUL, CampoConsulta::MODO_COLUNA, false, null);
        $oOdTipo->addComparacao('Cotação', CampoConsulta::COMPARACAO_IGUAL, CampoConsulta::COR_AMARELO, CampoConsulta::MODO_COLUNA, false, null);

        $oOdTipo->setBComparacaoColuna(true);

        $oClienteF = new Filtro($oCliente, Campo::TIPO_TEXTO, 3, 3, 12, 12, false);

        $this->addFiltro($oClienteF, $oContatoFiltro, $oOdNr);

        $this->setUsaDropdown(true);
        $oDrop1 = new Dropdown('Movimentação', Dropdown::TIPO_SUCESSO);
        $oDrop1->addItemDropdown($this->addIcone(Base::ICON_CONFIG) . 'Libera Faturamento', 'Od', 'liberaFat', '', false, false, '', false, '', false, false);
        $oDrop1->addItemDropdown($this->addIcone(Base::ICON_DELETAR) . 'Encerra sem Faturamento', 'Od', 'fechaOdmsg', '', false, false, '', false, '', false, false);
        $oDrop1->addItemDropdown($this->addIcone(Base::ICON_DESBLOQUEADO) . 'Retorna situação', 'Od', 'retAbertaMsg', '', false, false, '', false, '', false, false);

        $oDrop2 = new Dropdown('Vizualizar', Dropdown::TIPO_PRIMARY);
        $oDrop2->addItemDropdown($this->addIcone(Base::ICON_CONFIG) . 'Vizualizar Ordem', 'Od', 'acaoMostraRelConsulta', '', false, 'mostraordem', false, '', false, '', false, false);
