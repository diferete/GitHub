...
class ViewPoliManut extends View{
    ...
    public function criaConsulta() {
        ...
        $oSituaca->addComparacao('Aberta', CampoConsulta::COMPARACAO_IGUAL, CampoConsulta::COR_AZUL, CampoConsulta::MODO_LINHA);
        $oSituaca->addComparacao('Cancelada', CampoConsulta::COMPARACAO_IGUAL, CampoConsulta::COR_VERMELHO, CampoConsulta::MODO_LINHA);
        $oSituaca->addComparacao('Iniciada', CampoConsulta::COMPARACAO_IGUAL, CampoConsulta::COR_VERDE, CampoConsulta::MODO_LINHA);
        
          
        $this->setUsaDropdown(true);
        $oDrop1 = new Dropdown('Imprimir', Dropdown::TIPO_PRIMARY);
        $oDrop1->addItemDropdown($this->addIcone(Base::ICON_IMPRESSORA).'Ficha para mecânico', 'PoliManut', 'acaoMostraRelConsulta','', false, 'fichamanutpoli');
        $oDrop1->addItemDropdown($this->addIcone(Base::ICON_PASTA).'Ordem completa', 'PoliManut', 'acaoMostraRelConsulta','', false, 'ordemmanutpoli');
        
        $oDrop2 = new Dropdown('Movimentações', Dropdown::TIPO_INFO);
        $oDrop2->addItemDropdown($this->addIcone(Base::ICON_EDITAR).'Aponta início da manutenção', 'PoliManut', 'msgLibManut','', false);
        $oDrop2->addItemDropdown($this->addIcone(Base::ICON_FECHAR).'Encerramento', 'PoliManut', 'msgEnc','', false);
        $oDrop2->addItemDropdown($this->addIcone(Base::ICON_LOOP).'Retorna para aberta', 'PoliManut', 'msgRetAberta','', false);
        $oDrop2->addItemDropdown($this->addIcone(Base::ICON_BORRACHA).'Cancela', 'PoliManut', 'msgCancela','', false);
        
        $this->addDropdown($oDrop1, $oDrop2); 
        
        $oFnr = new Filtro($oNr, Campo::TIPO_TEXTO,1);
        $oFMaquina = new Filtro($oMaquina, Campo::TIPO_TEXTO,3);
