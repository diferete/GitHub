...
foreach ($aDados as $skey){
    ...
    $sSql2 = "select corrida, MetQual_MovOi .procod,prodes from MetQual_MovOi 
              ...
              where corrida like '".$skey."' ";
    $sth2 = $PDO->query($sSql2);
    $row2='';
    $row2 = $sth2->fetch(PDO::FETCH_ASSOC);
   
    $pdf->Cell(102, 4, rtrim($skey).'/ '.rtrim($row2['procod']).' - '.rtrim($row2['prodes']), 1, 1, 'C',1);

}
$pdf->SetXY($x+102, $y);

$pdf->Cell(102, 12, $row['codprod'].' - '.$row['prodes'], 1, 1, 'C',1);
$pdf->SetFont('Arial', 'B', 8);
...
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(40, 5, 'FUNCIONARIO:', 'B,T,L', 0, 'L',1);
$pdf->SetFont('Arial', '', 8);
$pdf->Cell(62, 5, $row['usercausa'], 'B,T,R', 0, 'L',1);
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(40, 5, 'FUNCIONARIO:', 'B,T,L', 0, 'L',1);
$pdf->SetFont('Arial', '', 8);
$pdf->Cell(62, 5, $row['userini'], 'B,T,R', 1, 'L',1);
...
$pdf->Cell(204, 5, 'OBSERVAÇÃO:', 'R,L,T', 1, 'L',1);
$pdf->SetFont('Arial', '', 8);
$pdf->MultiCell(204, 6, $row['descdescirnc'], 'R,L,B', 1, 'L',1);

$pdf->Ln(3);

$pdf->SetFont('Arial', 'B', 10);
$pdf->SetFillColor(180);
$pdf->Cell(204, 6, 'CONHECIMENTOS NOTIFICADOS (Resposáveis: Assinaturas)', 0, 1, 'C',1);
$pdf->SetFillColor(255);
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(164, 8, 'CAUSADOR DA NÃO CONFORMIDADE ('.$row['usercausa'].'):', 1, 0, 'L',1);
$pdf->Cell(40, 8, 'DATA:', 1, 1, 'L',1);
$pdf->Ln(1);
$pdf->Cell(164, 8, 'LÍDER DO SETOR CAUSADOR ('.$row['lidercausa'].'):', 1, 0, 'L',1);
$pdf->Cell(40, 8, 'DATA:', 1, 1, 'L',1);
$pdf->Ln(1);
$pdf->Cell(164, 8, 'GERENTE DA PRODUÇÃO (Volnei Hoffmann):', 1, 0, 'L',1);
$pdf->Cell(40, 8, 'DATA:', 1, 1, 'L',1);
$pdf->Ln(1);
$pdf->Cell(164, 8, 'LABORATÓRIO ('.$row['userf'].'):', 1, 0, 'L',1);
$pdf->Cell(40, 8, 'DATA:', 1, 1, 'L',1);
$pdf->Ln(1);
$pdf->Cell(164, 8, 'OUTRO:', 1, 0, 'L',1);
$pdf->Cell(40, 8, 'DATA:', 1, 1, 'L',1);
$pdf->Ln(1);
$pdf->Cell(164, 8, 'RESPONSÁVEL DO SETOR ('.$row['respcausa'].'):', 1, 0, 'L',1);
$pdf->Cell(40, 8, 'DATA:', 1, 1, 'L',1);
$pdf->Ln(1);
$pdf->Cell(204, 8, 'Nº O.R.::', 1, 1, 'L',1);
