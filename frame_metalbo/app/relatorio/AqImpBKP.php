...
$sSql = "select COUNT(*)as total from tbacaoeficaz where nr = '" . $nrAq . "'";
...
if ($eficaz['total'] == 0) {
    ...
    $sSql = " select COUNT(*) total2 from tbacaoeficaz where nr = '" . $nrAq . "' and eficaz <> 'Sim'";
...
$sSql = "select certificacao,userimp,convert(varchar,dtimp,103) as dtimp,titulo,usunome,equipe,convert(varchar,dataini,103) as dataini, "
        ...
        . "pq1,pq2,pq3,pq4,pq5 "
        . " from tbacaoqual where filcgc ='" . $FilcgcRex . "' and nr=" . $nrAq;
...
$sSqlContencao = "select COUNT(*) as total "
        ...
        . "where filcgc = '" . $FilcgcRex . "' and nr ='" . $nrAq . "'";
...
if ($existeCorrecao['total'] > 0) {
    $sSqlDadosCorrecao = "select correcao,convert(varchar,dataprev,103) as dataprev,usunome,situaca "
            . "from MET_QUAL_Correcao "
            . "where filcgc = '" . $FilcgcRex . "' and nr ='" . $nrAq . "'";
    ...
    while ($rowCorrecao = $dadosCorrecao->fetch(PDO::FETCH_ASSOC)) {
        ...
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->MultiCell(206, 5, 'Ação efetuada = ' . $rowCorrecao['correcao'], 1, 'L');
        $iAlturaAcao = $pdf->GetY();
    }
}
...
$sSqlC = "select matprimades, metododes, maodeobrades, "
        ...
        . " from MET_QUAL_DiagramaCausa where nr='" . $nrAq . "' and filcgc ='" . $FilcgcRex . "' ";
...
$sSql = "select causades,pq1,pq2,pq3,pq4,pq5,seq from tbacaoqualcausa where nr=" . $nrAq . " and filcgc ='" . $FilcgcRex . "' order by seq";
...
$sSql = "select seq,plano,convert(varchar,dataprev,103) as dataprev,usunome,convert(varchar,datafim,103) as datafim,procedimento,it,planocontrole,fluxograma,ppap,contexto,preventiva,funcao,treinamento,tbacaoqualplan.obsfim "
        . " from tbacaoqualplan where nr=" . $nrAq . " and filcgc ='" . $FilcgcRex . "' and tipo <> 'Eficiência' order by seq";
...
$sSql = "select seq,acao,convert(varchar,dataprev,103) as dataprev,"
        ...
        . "from tbacaoeficaz where nr =" . $nrAq . " and filcgc ='" . $FilcgcRex . "' order by seq";
...
while ($row = $dadosEficaz->fetch(PDO::FETCH_ASSOC)) {
    ...
    if ($row['comAcao'] == 'S') {
        ...
        $sSql = "select plano,convert(varchar,dataprev,103) as dataprev,usunome,convert(varchar,datafim,103) as datafim"
                . " from tbacaoqualplan where nr=" . $nrAq . " and filcgc ='" . $FilcgcRex . "' "
