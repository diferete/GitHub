...
//        }
     ...
     */
    public function mostraConsultaDetail($sRenderTo = 'Ext.getBody()', $bConsultaBusca = false, $sCampoRetorno = "", $sCampoForm = "", $bAdicionaAcoesPadrao = true, $sCamposFiltrosExtra = "", $sValoresFiltrosExtra = "") {
            ...
            if (isset($aFiltros)) {
                ...
                foreach ($aFiltros as $key => $aFiltroGrid) {
                        ...
                        if ($bReload !== TRUE) {
                            ...
                            if ($aFiltroGrid[2] == 'entre') {
                                ...
                                if ($aFiltroGrid[3] == 'vlrini') {
                                    $aEntre[$iCont]['vlrini'] = $aFiltroGrid[1];
                                    $aEntre[$iCont]['vlrfim'] = null;
                                };
                                if ($aFiltroGrid[3] == 'vlrfim') {
                                    $aEntre[$iCont]['vlrini'] = null;
                                    $aEntre[$iCont]['vlrfim'] = $aFiltroGrid[1];
                                };
                    ...
                    if ($aFiltroGrid[1] == 'scroll') {
                        ...
                        /* $sModel = $this->Persistencia->getCampoByNameModel($key);
                         * if(count($aMetodos) > 1){
                          if($key != count($aMetodos) - 1){
                          $oModel = $oModel->$sMetodoGetter();
                          } else{
...
// $this->Persistencia->adicionaFiltro($key,$value,Persistencia::LIGACAO_AND, $this->tipoFiltro($aFiltroGrid[1])); 
                        /*  $aScroll = explode('=',$aFiltroGrid[0]);
                          $this->Persistencia->adicionaFiltro($aScroll[0],$aScroll[1],Persistencia::LIGACAO_AND, $this->tipoFiltro($aFiltroGrid[1])); */
                    }
                }
                ...
                foreach ($aEntre as $aEntreFiltro) {
                    ...
                    if (isset($aEntreFiltro['vlrini'])) {
                        $sVlrInicial = $aEntreFiltro['vlrini'];
                    };
                    if (isset($aEntreFiltro['vlrfim'])) {
                        $sVlrFinal = $aEntreFiltro['vlrfim'];
                    };
                }
                if (!empty($aEntre)) {
        ...
        foreach ($aModels as $oAtual) {
            ...
            if ($aRetornoFormat[0]) {
                $sDados .= '<tr class="' . $aRetornoFormat[1] . ' dbclick" style="font-size:small;">';
            } else {
                $sDados .= '<tr role="row" class="odd dbclick" style="font-size:small;">'; //abre a linha
            }
    ...
    public static function getListaFrom($sClasse, $sMetodo, $sCampoValor, $sCampoDescricao, $bOrdena = true, $sWhere = null) {
        ...
        if ($sMetodo === null) {
            ...
            foreach ($aListaWhere as $aAtual) {
                $oPersistencia->adicionaFiltro($aAtual['campo'], $aAtual['valor'], $aAtual['ligacao'], $aAtual['comparacao'], $aAtual['valorFim']);
            }
