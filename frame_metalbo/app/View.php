    ...
    public function addBotaoPadraoTela($sCampoIncremento) {
        ...
        if ($this->getBEtapa()) {
            ...
            if (!($this->getBOcultaBotTela())) {
                ...
                $oBtnAdd->setRequestAjax('requestAjax("' . $this->getTela()->getId() . '-form","' . $sClasse . '","' . $sMetodo . '","' . $this->getTela()->getId() . ',' . $this->getTela()->getSRenderHide() . ',' . $sCampoIncremento . ','.$this->View->getTela()->getAbaSel().'");'); //"'.$this->getSIdUpload().'"
                $this->getTela()->addBotoes($oBtnAdd);
            }
